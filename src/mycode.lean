...
meta def rebuild_context_modified : (list context.decl) -> tactic ((name_map context.decl) × (name_map local_context) × (list context.decl))
| [] := return (mk_name_map, mk_name_map, [])
| (d :: ds) := do
  (nm_map, loc_map, ds') <- rebuild_context_modified ds,
  (nm_map, loc_map, d') <- build_context_aux nm_map loc_map d,
  return (nm_map, loc_map, d' :: ds')
...
| (d1 :: d2 :: ds) (context.decl.mvar_decl g) := do
  ...
  (nm_map, loc_map, (d1'::d2'::ds')) <- rebuild_context_modified (d1 :: d2 :: ds),
  tactic.trace format! "[rebuild_context_with_goal] d1' {d1'}",
  tactic.trace format! "[rebuild_context_with_goal] d2' {d2'}",
  -- update dependencies
  ...
  let ld := {local_decl .
      unique_name := new_uid,
      pp_name := pp_name,
      type := g.type.get_or_else g.expr_type,
      value := none,
      bi := bi,
      idx := 1,
      },
  tactic.trace format! "[rebuild_context_with_goal] ld {ld}",
  new_assignment <- ld.value.mmap ((swap_mvs nm_map) ∘ expr.unfold_string_macros ∘ expr.erase_annotations),
  ...
  let new_lcxt := new_lcxt_option.get_or_else local_context.empty,
  -- tactic.trace format! "[rebuild_context_with_goal] new_lcxt {new_lcxt}",
  ...
  -- | _ := none
  -- end,
  let new_decl := match d2 with
  | (context.decl.local_decl dd) := some (context.decl.local_decl {
      unique_name := new_uid,
      pp_name := pp_name,
      expr_type := dd.expr_type,
      bi := bi,
      prev := new_prev_id,
      type := g.type,
      frozen := tt,
  ...
  -- tactic.trace format! "[rebuild_context_with_goal] d2 {d2}",
  tactic.trace format! "[rebuild_context_with_goal] new_decl {new_decl}",
  let d := (new_decl.get_or_else d2),
  let new_nmap := nm_map.insert new_uid (new_decl.get_or_else d2),
  let new_loc_map := loc_map.insert new_uid new_lcxt,
  -- build_context_aux nm_map loc_map g,
  (nm_map, loc_map) <- rebuild_context (d::d2::ds),
  (nm_map, loc_map, _) <- build_context_aux nm_map loc_map d1,
  return (nm_map, loc_map)
...
meta def rebuild_tactic_state_with_goal (ts : tactic_state_data) : tactic unit := do
  ...
  -- tactic.trace format!"[rebuild_tactic_state] ds: {ds}",
  let goals := goals_and_tags.map prod.fst,
  (nm_map, _) <- rebuild_context_with_goal ts.decls.reverse (ds.reverse.head),
  tactic.trace format!"[rebuild_tactic_state] nm_map: {nm_map}",
...
-- 
meta def finish_goals : tactic unit := do
  let jsn := (json.array ([
    json.array ([
      json.of_string "name.mk_string",
      json.of_string "mk",
      json.array ([
        json.of_string "name.mk_string",
        json.of_string "tactic_state_data",
