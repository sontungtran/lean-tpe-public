      ...
      (has_from_json.from_json un) <*> 
        (has_from_json.from_json pp) <*> 
          has_from_json.from_json ety <*> 
            has_from_json.from_json cxt <*> 
              has_from_json.from_json ty <*> 
                has_from_json.from_json assn
    else
...
-- attribute [derive [has_to_tactic_json]] json
...
⟨λ msg, match msg with
  | (json.array $ [c, json.array [un, pp, ty, val, bi, idx]]) := do
    tactic.trace format!"[has_from_json_context.decl] bi: {bi}, {idx}",
    (c_nm : name) ← has_from_json_name_aux c,
...
| (context.decl.local_decl d) := do
  ...
  tactic.trace format! "[rebuild_context_aux] new_decl: {new_decl}",
  let new_nmap := nm_map.insert d.unique_name new_decl,
...
| (context.decl.mvar_decl d) := do
  ...
  tactic.trace format!"[build_context_aux] New_decl: {new_decl}",
  let new_nmap := nm_map.insert d.unique_name new_decl,
  return (new_nmap, loc_map, new_decl)
...
meta def rebuild_context_with_goal : (list context.decl) -> decl -> tactic ((name_map context.decl) × (name_map local_context))
| [] g := return (mk_name_map, mk_name_map)
| (d1 :: d2 :: ds) (context.decl.mvar_decl g) := do
  tactic.trace format!"[rebuild_context_with_goal] Goal: {g}",
  tactic.trace format!"[rebuild_context_with_goal] Prev ds: {d1} {d2}",
  (nm_map, loc_map) <- rebuild_context (d1 :: d2 :: ds),
  -- update dependencies
  let new_uid := "0._fresh.4995.1451",
  let pp_name := "h100",
  let bi := binder_info.default,
  -- let new_tp := expr'.to_expr (expr'.var 1),
  let new_tp := swap_mvs nm_map (g.type.get_or_else g.expr_type).unfold_string_macros.erase_annotations,
  let ld := {local_decl .
      unique_name := new_uid,
      pp_name := pp_name,
      type := g.expr_type,
  ...
  -- create local 
  -- let new_lcxt := local_context.empty,
  -- let new_lcxt := g.local_cxt,
  -- match d with 
  -- | some e := type_context.run $ type_context.assign new_mvar e
  -- | none := return ()
  -- end,
  let (new_lcxt_option : option local_context) := do {
    -- unique_name <- ,
    loc_map.find $ d2.unique_name
  },
  let new_lcxt := new_lcxt_option.get_or_else local_context.empty,
  
  (new_loc, new_lcxt) <- new_lcxt.mk_local2 pp_name g.expr_type ld.bi tt new_assignment,
  -- let new_tp := eq.{1} nat (nat.succ a_n) (nat.succ a_n),
  let new_prev_id := some (d2.unique_name),
  let ld := (some {local_decl .
      unique_name := new_uid,
      pp_name := pp_name,
      type := g.expr_type,
      value := none,
  ...
  -- let pp_name := g.pp_name,
  new_type <- swap_mvs nm_map (g.type.get_or_else g.expr_type).unfold_string_macros.erase_annotations,
  -- let (new_lcxt_option : option local_context) := do {
  --   unique_name <- new_prev_id,
  --   loc_map.find unique_name
  -- },
  -- let new_lcxt := new_lcxt_option.get_or_else local_context.empty,
  -- new_assignment <- ld.value.mmap ((swap_mvs nm_map) ∘ expr.unfold_string_macros ∘ expr.erase_annotations),
  -- -- create local 
  -- (new_loc, new_lcxt) <- new_lcxt.mk_local2 pp_name new_type d.bi tt new_assignment,
  -- (new_uid, new_tp) <- match new_loc with
  -- | expr.local_const nm _ bi tp := return (nm, tp)
  -- | _ := tactic.failed
  -- end,
  ...
  let new_nmap := nm_map.insert new_uid new_decl,
  let new_loc_map := loc_map.insert new_uid new_lcxt,
  -- build_context_aux nm_map loc_map g,
  return (nm_map, loc_map)
| ds _ := tactic.fail "Deamn"
...
meta def rebuild_tactic_state (ts : tactic_state_data) : tactic unit := do
  ...
  -- tactic.trace format!"[rebuild_tactic_state] nm_map {nm_map}",
  goals_and_tags <- ts.goals.mmap $ λ ⟨nm, tag⟩, do {
    tactic.trace format!"[rebuild_tactic_state] nm: {nm} tag: {tag}",
    d <- nm_map.find nm,
    tactic.trace format!"[rebuild_tactic_state] d {d}",
    nm' <- match d with
...
meta def rebuild_tactic_state_with_goal (ts : tactic_state_data) : tactic unit := do
  (nm_map, _) <- rebuild_context ts.decls.reverse,
  -- tactic.trace format!"[rebuild_tactic_state] nm_map {nm_map}",
  goals_and_tags <- ts.goals.mmap $ λ ⟨nm, tag⟩, do {
    tactic.trace format!"[rebuild_tactic_state] nm: {nm} tag: {tag}",
    ...
    -- tactic.trace format!"[rebuild_tactic_state] d {d}",
    type <- match d with
    | context.decl.mvar_decl dd := dd.type
    | _ := tactic.fail "Expecting mvar_decl"
    end,
    nm' <- match d with
    | context.decl.mvar_decl dd := return dd.unique_name
    ...
    tactic.trace format!"[rebuild_tactic_state] type: {type}",
    mvars <- type_context.run type_context.list_mvars,
    mv <- mvars.mfirst $ λ e, do {
      nm2 <- mvar_id e,
      if nm' = nm2 then return e else failure
  ...
  let goals := goals_and_tags.map prod.fst,
  (nm_map, _) <- rebuild_context_with_goal ts.decls.reverse (ds.head),
  tactic.trace format!"[rebuild_tactic_state] nm_map: {nm_map}",
  tactic.set_goals goals,
  goals_and_tags.mmap $ λ ⟨g, tag⟩, do {
    tactic.enable_tags tt,
...
-- examples
meta def trace_context_1 : tactic unit := do
  -- cxt <- context.get,
  -- cxt ← tactic_state_data.get,
  -- has_from_json.from_json (json.array ([json.array (["name.mk_string", "mk", json.array (["name.mk_string", "tactic_state_data", json.array (["name.anonymous"])])]), json.array ([json.array ([json.array (["name.mk_string", "nil", json.array (["name.mk_string", "list", json.array (["name.anonymous"])])])]), json.array ([json.array (["name.mk_string", "nil", json.array (["name.mk_string", "list", json.array (["name.anonymous"])])])])])])),
  -- let jsn := (json.array ([json.array (["name.mk_string", "mk", json.array (["name.mk_string", "tactic_state_data", json.array (["name.anonymous"])])]), json.array ([json.array ([json.array (["name.mk_string", "nil", json.array (["name.mk_string", "list", json.array (["name.anonymous"])])])]), json.array ([json.array (["name.mk_string", "nil", json.array (["name.mk_string", "list", json.array (["name.anonymous"])])])])])])),
  -- let jsn := (),
  let jsn := (json.array ([json.array ([json.of_string "name.mk_string", json.of_string "mk", json.array ([json.of_string "name.mk_string", json.of_string "tactic_state_data", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "local_decl", json.array ([json.of_string "name.mk_string", json.of_string "decl", json.array ([json.of_string "name.mk_string", json.of_string "context", json.array ([json.of_string "name.anonymous"])])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "mk", json.array ([json.of_string "name.mk_string", json.of_string "local_decl2", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1083, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_string", json.of_string "α", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1, json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "default", json.array ([json.of_string "name.mk_string", json.of_string "binder_info", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "some", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "sort", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "succ", json.array ([json.of_string "name.mk_string", json.of_string "level", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "zero", json.array ([json.of_string "name.mk_string", json.of_string "level", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "none", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.of_bool tt, json.array ([json.array ([json.of_string "name.mk_string", json.of_string "some", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "mk", json.array ([json.of_string "name.mk_string", json.of_string "local_decl", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1083, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_string", json.of_string "α", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "sort", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "succ", json.array ([json.of_string "name.mk_string", json.of_string "level", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "zero", json.array ([json.of_string "name.mk_string", json.of_string "level", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "none", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "default", json.array ([json.of_string "name.mk_string", json.of_string "binder_info", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.of_int 0])])])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "mvar_decl", json.array ([json.of_string "name.mk_string", json.of_string "decl", json.array ([json.of_string "name.mk_string", json.of_string "context", json.array ([json.of_string "name.anonymous"])])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "mk", json.array ([json.of_string "name.mk_string", json.of_string "mvar_decl", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1096, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_string", json.of_string "_mlocal", json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_numeral", json.of_int 1096, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_string", json.of_string "_mlocal", json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 2, json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "local_const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1083, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_string", json.of_string "α", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "default", json.array ([json.of_string "name.mk_string", json.of_string "binder_info", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1, json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "some", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "app", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "app", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "app", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "eq", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "succ", json.array ([json.of_string "name.mk_string", json.of_string "level", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "zero", json.array ([json.of_string "name.mk_string", json.of_string "level", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nat", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "zero", json.array ([json.of_string "name.mk_string", json.of_string "nat", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "zero", json.array ([json.of_string "name.mk_string", json.of_string "nat", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "none", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "local_decl", json.array ([json.of_string "name.mk_string", json.of_string "decl", json.array ([json.of_string "name.mk_string", json.of_string "context", json.array ([json.of_string "name.anonymous"])])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "mk", json.array ([json.of_string "name.mk_string", json.of_string "local_decl2", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1099, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_string", json.of_string "a_n", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1, json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "default", json.array ([json.of_string "name.mk_string", json.of_string "binder_info", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "none", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "some", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1083, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])])])]), json.of_bool tt, json.array ([json.array ([json.of_string "name.mk_string", json.of_string "some", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "mk", json.array ([json.of_string "name.mk_string", json.of_string "local_decl", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1099, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_string", json.of_string "a_n", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nat", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "none", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "default", json.array ([json.of_string "name.mk_string", json.of_string "binder_info", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.of_int 1])])])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "local_decl", json.array ([json.of_string "name.mk_string", json.of_string "decl", json.array ([json.of_string "name.mk_string", json.of_string "context", json.array ([json.of_string "name.anonymous"])])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "mk", json.array ([json.of_string "name.mk_string", json.of_string "local_decl2", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1100, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_string", json.of_string "a_ih", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1, json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "default", json.array ([json.of_string "name.mk_string", json.of_string "binder_info", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "none", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "some", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1099, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])])])]), json.of_bool tt, json.array ([json.array ([json.of_string "name.mk_string", json.of_string "some", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "mk", json.array ([json.of_string "name.mk_string", json.of_string "local_decl", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1100, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_string", json.of_string "a_ih", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "app", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "app", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "app", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "eq", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "succ", json.array ([json.of_string "name.mk_string", json.of_string "level", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "zero", json.array ([json.of_string "name.mk_string", json.of_string "level", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nat", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "local_const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1099, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_string", json.of_string "a_n", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "default", json.array ([json.of_string "name.mk_string", json.of_string "binder_info", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1, json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "local_const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1099, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_string", json.of_string "a_n", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "default", json.array ([json.of_string "name.mk_string", json.of_string "binder_info", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1, json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "none", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "default", json.array ([json.of_string "name.mk_string", json.of_string "binder_info", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.of_int 1])])])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "mvar_decl", json.array ([json.of_string "name.mk_string", json.of_string "decl", json.array ([json.of_string "name.mk_string", json.of_string "context", json.array ([json.of_string "name.anonymous"])])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "mk", json.array ([json.of_string "name.mk_string", json.of_string "mvar_decl", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1101, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_string", json.of_string "_mlocal", json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_numeral", json.of_int 1101, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_string", json.of_string "_mlocal", json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 2, json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "local_const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1083, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_string", json.of_string "α", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "default", json.array ([json.of_string "name.mk_string", json.of_string "binder_info", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1, json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "local_const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1099, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_string", json.of_string "a_n", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "default", json.array ([json.of_string "name.mk_string", json.of_string "binder_info", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1, json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "local_const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1100, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_string", json.of_string "a_ih", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "default", json.array ([json.of_string "name.mk_string", json.of_string "binder_info", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1, json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "some", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "app", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "app", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "app", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "eq", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "succ", json.array ([json.of_string "name.mk_string", json.of_string "level", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "zero", json.array ([json.of_string "name.mk_string", json.of_string "level", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nat", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "app", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "succ", json.array ([json.of_string "name.mk_string", json.of_string "nat", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "local_const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1099, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_string", json.of_string "a_n", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "default", json.array ([json.of_string "name.mk_string", json.of_string "binder_info", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1, json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "app", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "succ", json.array ([json.of_string "name.mk_string", json.of_string "nat", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "local_const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1099, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.of_string "name.mk_string", json.of_string "a_n", json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "default", json.array ([json.of_string "name.mk_string", json.of_string "binder_info", json.array ([json.of_string "name.anonymous"])])]), json.array ([])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "const", json.array ([json.of_string "name.mk_string", json.of_string "expr'", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1, json.array ([json.of_string "name.anonymous"])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])])])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "none", json.array ([json.of_string "name.mk_string", json.of_string "option", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])])])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "mk", json.array ([json.of_string "name.mk_string", json.of_string "prod", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1096, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_string", json.of_string "_mlocal", json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "hyps", json.array ([json.of_string "name.mk_string", json.of_string "_case", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "zero", json.array ([json.of_string "name.mk_string", json.of_string "nat", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.array ([json.of_string "name.mk_string", json.of_string "mk", json.array ([json.of_string "name.mk_string", json.of_string "prod", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_numeral", json.of_int 1101, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_string", json.of_string "_mlocal", json.array ([json.of_string "name.anonymous"])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "hyps", json.array ([json.of_string "name.mk_string", json.of_string "_case", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "_arg", json.array ([json.of_string "name.mk_numeral", json.of_int 1099, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "_arg", json.array ([json.of_string "name.mk_numeral", json.of_int 1100, json.array ([json.of_string "name.mk_numeral", json.of_int 225, json.array ([json.of_string "name.mk_string", json.of_string "_fresh", json.array ([json.of_string "name.mk_numeral", json.of_int 0, json.array ([json.of_string "name.anonymous"])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "cons", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "succ", json.array ([json.of_string "name.mk_string", json.of_string "nat", json.array ([json.of_string "name.anonymous"])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])])])])])])])]), json.array ([json.array ([json.of_string "name.mk_string", json.of_string "nil", json.array ([json.of_string "name.mk_string", json.of_string "list", json.array ([json.of_string "name.anonymous"])])]), json.array ([])])])])])])])])),
  -- tactic.trace format!"[has_from_json_context.decl] c_nm: {jsn}",
  -- tactic.trace format!"[has_from_json_context.decl]",
  -- -- has_from_json_name_aux jsn,
  let ⟨fn₁⟩ := (by apply_instance : has_from_json (list context.decl)), 
  let ⟨fn₂⟩ := (by apply_instance : has_from_json (list (name × tactic.tag))),

  msg ← match jsn with
  | (json.array $ [c, json.array [decls_msg, goals_msg]]) := do
    tactic.trace format!"Bruh",
    (c_nm : name) ← has_from_json_name_aux c,
    -- tactic.trace format!"[has_from_json_tactic_state_data] c_nm: {c_nm}, goal: {goals_msg}",
    if c_nm = `tactic_state_data.mk then tactic_state_data.mk <$> fn₁ decls_msg <*> fn₂ goals_msg else
    tactic.fail format!"[has_from_json_tactic_state_data] unexpected: "
  | exc := tactic.fail format!"[has_from_json_tactic_state_data] unexpected: {exc}"
  end,
  -- tactic.trace format!"{msg}",
  rebuild_tactic_state_with_goal msg
  -- rebuild_tactic_state msg
  -- new_local <- tactic.intro_core "h100",
  -- cxt ← tactic_state_data.get
  -- set_state msg,
  -- (c_nm : name) ← has_from_json_name_aux jsn,
  -- tactic.trace format!"[has_from_json_tactic_state_data] c_nm: {c_nm}",
  -- if c_nm = `tactic_state_data.mk then tactic_state_data.mk <$> fn₁ decls_msg <*> fn₂ goals_msg else
  -- tactic.fail format!"[has_from_json_tactic_state_data] unexpected: {msg}"
  -- has_from_json.from_json jsn,
  -- has_to_tactic_json.to_tactic_json cxt >>= tactic.trace
...
-- -- Debug
-- example {α β γ : Type} (f : α → β) (h : α → β) (h1: f = h)
--   : f = h :=
-- begin
--   skip,
--   trace_context_1,
